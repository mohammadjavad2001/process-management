...
public class psjf {
    ...
    public void findavgTime(ArrayList<ProcessClass> list,ArrayList<ProcessClass> queue)
    {
       ...
       // int finishtime[]=new int[100];
        ...
        for (int j = 0; j < 1000; j++) {
            ...
            psjf Executer=new psjf();
            temp= Executer.psjfExecuter(queue, wt, Response,temp[0],temp[1]);

            psjffindTurnAroundTime(queue, wt, tat);
        ...
        //temp[0]=total time temp[1]= utiliz
        float avgthroughput=(float)100000/(float)temp[0];

        float utilization1=((float)temp[0]+(float) temp[1])/(float)temp[0] ;
    ...
    static int[] psjfExecuter(ArrayList<ProcessClass> queue,int wt[],int Response[],int t,int utiliz/*,0int finishtime[]*/ ) {
        ...
        while (complete != queue.size()) {
            ...
            if (brusttime[shortest] == queue.get(shortest).getPBurstTime() ) {
                Response[shortest] = t;
            }
            if (check == false) {
                utiliz--;
            ...
            if (brusttime[shortest] == 0) {
                ...
                // process
                finish_time = t + 1;
              //  finishtime[shortest]=finish_time;
     ...
     //   System.out.println(utiliz+" dwdmmmmmmmmmmmmmmmm");

        int[] a = {t,utiliz};
        return a;

    }
    static void psjffindTurnAroundTime(ArrayList<ProcessClass> queue, int wt[], int tat[])
    {
